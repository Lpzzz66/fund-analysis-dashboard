...
- 已完成任务租约隔离、导入任务状态/重试/校验/原始来源接口和独立 `app.worker`（任务进程）入口。
- 已完成任务九的只读 IMAP（邮件接收协议）同步、附件幂等、异常隔离、原始文件一年滚动清理和数据库备份适配器；授权码只从环境变量或文件引用读取。
- 已完成任务十的历史目录清单、主目录优先去重、`gz`（归档目录）补充候选、冲突复核、预演、断点上传和迁移报告；源目录始终只读。
- 本批次质量门禁：后端 184 项测试、历史盘点工具 63 项测试通过；Ruff（静态检查）、格式检查、ty（静态类型检查）、pip check（依赖检查）和 SQLite（本地数据库）迁移升级/回退/再升级通过。
