...
worker 领取租约 -> 读取 Excel -> 识别身份 -> 标准化落库
        |
        v
校验 -> 无问题：自动发布；有 warning：publishable；有 critical：pending_review / failed / non_valuation
        |
        +--> 干净版本自动发布 -> 每产品一个范围合并的 background_job(process_analysis_run)
        |
        +--> 警告版本人工确认发布 / 阻断版本人工复核 -> background_job(process_analysis_run)
        |
        v
...
导入或分析业务写入与租约完成在同一调用事务中提交。租约失效时业务写入回滚，旧 worker 不能覆盖新 worker 的结果。当前 worker 默认串行领取任务。
...
发布服务只负责改变版本状态并创建分析任务，不在发布 HTTP 请求内重算全部历史。导入批次中的干净版本由批处理调用同一个事务发布服务自动发布，并在批次末尾按产品把自动发布日期范围合并为一个分析任务；警告版本和人工发布、撤回、恢复仍按单次操作创建分析任务。分析 worker 读取所有已发布记录作为历史上下文，仅把受影响日期范围的产品指标、公司指标和风险事件写入本次 `analysis_run`。分析任务已经由 `worker.py`（后台任务入口）实际执行，不是仅有占位记录。
