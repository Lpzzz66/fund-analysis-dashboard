...
- [生产运行手册](runbook.md)：首次部署、迁移、管理员初始化、worker、Caddy、备份恢复、清理、监控和升级回滚。
...
- [Excel 能力边界与前端 Demo 审计](07-Excel能力边界与前端Demo审计.md)：基于 2,787 张历史估值表核验数据边界、解析缺口、页面承诺和前端模拟功能。
- [前后端真实接入验收报告](08-前后端真实接入验收报告.md)：记录正式前端页面、后端接口、权限、安全边界和自动化验收结果。
...
开发 compose（本地容器编排）使用 `APP_HOST` 和 `APP_PORT` 启动后端，宿主机端口仅绑定到 `127.0.0.1`（本机回环地址）。生产 compose 位于 `deploy/compose.prod.yml`，必须显式使用 `--env-file deploy/.env`，不使用仓库中的真实秘密。
...
本批次验证结果：后端和历史盘点工具测试、Ruff（静态检查）、格式化检查和 ty（静态类型检查）均已通过；SQLite（本地数据库）迁移升级、指定版本回退、非破坏性回退和再升级通过；三份真实 `.xls` 样本均可稳定解析。生产部署验收命令和当前 Docker（容器）环境限制见 `docs/runbook.md`。
...
每次推送和合并请求由 GitHub Actions（GitHub 自动化检查）执行以下检查：Ruff（静态检查）及格式检查、ty（静态类型检查）、后端测试和历史盘点工具测试。依赖版本继续由 `backend/pyproject.toml`（后端项目配置）中的上下界约束；真实凭据和生产配置不进入 CI（持续集成）或仓库。
