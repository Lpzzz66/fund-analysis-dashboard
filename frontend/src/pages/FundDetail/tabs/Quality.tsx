...
function FindingTable({ rows }: { rows: ImportValidationFinding[] }) {
  return <Table rowKey={(r) => r.rule_code} size="small" pagination={false} dataSource={rows} columns={[{ title: "规则编号", dataIndex: "rule_code" }, { title: "等级", dataIndex: "level", render: (v: ValidationLevel) => <LevelTag level={v} /> }, { title: "实际值", dataIndex: "actual_value", render: (v: string | null) => <Num>{dec(v, 2)}</Num> }, { title: "期望值", dataIndex: "expected_value", render: (v: string | null) => <Num>{dec(v, 2)}</Num> }, { title: "差异", dataIndex: "difference", render: (v: string | null) => <Num>{dec(v, 2)}</Num> }, { title: "来源位置", dataIndex: "source_location", render: (v: string | null) => <Truncate value={v ?? ""} /> }, { title: "说明", dataIndex: "message", render: (v: string | null) => <Truncate value={v ?? ""} /> }]} />;
}
