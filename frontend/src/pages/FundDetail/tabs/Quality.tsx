  ...
  return (
    <div>
      <div style={{ marginBottom: 12 }}>
        质量状态：<QualityBadge status={quality.quality_status} /> · 估值日：{quality.valuation_date ?? "—"}
      </div>
      {levels.map((level) => {
        const rows = quality.validation.filter((f) => f.level === level);
        return (
          <Card
            key={level}
            size="small"
            title={
              <Tag color={level === "critical" ? "error" : level === "warning" ? "warning" : "default"}>
                {VALIDATION_LEVEL_LABEL[level]} · {rows.length}
              </Tag>
            }
          ...
          >
            {rows.length ? (
              ...
              <Empty description={`无${VALIDATION_LEVEL_LABEL[level]}校验项`} image={Empty.PRESENTED_IMAGE_SIMPLE} />
            )}
          </Card>
        );
      })}
