...
export default function FundDetail() {
  ...
  return <div className="fd-page"><Card style={{ marginBottom: 12 }}><Space direction="vertical" size={16} style={{ width: "100%" }}><Space><h1 style={{ margin: 0, fontSize: 24 }}>{fund.name}</h1><Tag color={fund.status === "active" ? "success" : "default"}>{fund.status === "active" ? "启用" : "停用"}</Tag></Space><Row gutter={[10, 10]} className="fd-detail-metrics"><Col xs={12} sm={6}><div className="fd-kpi fd-detail-metric"><div className="fd-kpi__label">资产规模</div><div className="fd-kpi__value">{compactMoney(fund.total_assets)}</div></div></Col><Col xs={12} sm={6}><div className="fd-kpi fd-detail-metric"><div className="fd-kpi__label">资产净值</div><div className="fd-kpi__value">{compactMoney(fund.net_asset_value)}</div></div></Col><Col xs={12} sm={6}><div className="fd-kpi fd-detail-metric"><div className="fd-kpi__label">单位净值</div><div className="fd-kpi__value">{dec(fund.unit_nav, 4)}</div></div></Col><Col xs={12} sm={6}><div className="fd-kpi fd-detail-metric"><div className="fd-kpi__label">日收益 / 累计</div><div className="fd-kpi__value fd-detail-return"><span style={{ color: returnColor(fund.daily_return) }}>{pct(fund.daily_return)}</span><span style={{ color: returnColor(fund.cumulative_return) }}>{pct(fund.cumulative_return)}</span></div></div></Col></Row><Descriptions size="small" column={{ xs: 1, sm: 3 }}><Descriptions.Item label="产品代码">{fund.product_code ?? "—"}</Descriptions.Item><Descriptions.Item label="估值日">{dateStr(fund.valuation_date)}</Descriptions.Item><Descriptions.Item label="质量"><QualityBadge status={fund.quality_status} /></Descriptions.Item><Descriptions.Item label="分析状态">{fund.analysis_status}</Descriptions.Item></Descriptions></Space></Card><Tabs activeKey={activeTab} onChange={(key) => setParams((previous) => { const next = new URLSearchParams(previous); next.set("tab", key); return next; })} items={[{ key: "nav", label: "净值序列", children: <NavSeriesTab fundId={fundId} /> }, { key: "positions", label: "持仓", children: <PositionsTab fundId={fundId} /> }, { key: "quality", label: "数据质量", children: <QualityTab fundId={fundId} /> }]} /></div>;
}
